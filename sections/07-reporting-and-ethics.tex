...
This routing doesn't weaken ethics review; it aims it. A study that targets persons belongs in participant review with the usual protections; one that uses expert competence to assess linguistic materials belongs under the standards that govern measurement \citep{aera2014standards}. The routing for common designs is summarized in \autoref{tab:routing}.

\begin{table}[ht]
  \centering
  \caption{Routing judgment evidence by what the design studies. The role follows the focus of inquiry, so the same materials change rows as the research question changes, and a mixed design occupies more than one.}
  \label{tab:routing}
  \small
  \begin{tabular}{@{}p{0.28\textwidth}p{0.17\textwidth}p{0.43\textwidth}@{}}
    \toprule
    \textbf{What the design studies} & \textbf{Role} & \textbf{What to report} \\
    \midrule
